...
TODO list: 
    ...
    - remove printing holdout mse loss in dynamics model training (done)
    - Add TensorBoard support (done)
    - Add checkpoints for policy and dynamics model (done)
    - add function for testing the policy-dependence of the models (done)
    - resolving the increasing GPU memory usage issue (turns out not an issue with the current version of the code, oddly enough)
'''
...
def readParser():
    ...
    parser.add_argument('--save_dir', type=str, default='test_policy_dependent_results2/', metavar='A', 
                        help='directory for saving the tensorboard summary writer')
    ...
    parser.add_argument('--env_pool_save_fname', default='./test_policy_dependent_results_2/env_buffer/env_buffer_', metavar='A', 
                        help='filename for saving the environment trajectory buffer')
    parser.add_argument('--model_pool_save_fname', default='./test_policy_dependent_results_2/model_buffer/model_buffer_', metavar='A', 
                        help='filename for saving the dynamics model trajectory buffer')
    return parser.parse_args()


def train(args, env_sampler, predict_env, agent, env_pool, model_pool, writer, save_model_dir, save_policy_dir, save_env_pool_dir, save_dynamics_pool_dir):
    total_step = 0
    reward_sum = 0
    rollout_length = 1
            ...
            if total_step % 1000 == 0:
                ...
                for j in range(args.num_eval_episode):
                    ...
                    while not done:
                        ...
                        sum_reward += reward
                    logging.info('step reward: {} | inner eval num: {} | sum reward: {}'.format(total_step, j, sum_reward))
                ...
                logging.info('Average Step Reward: ' + str(total_step) + ' ' + str(avg_episode_reward/args.num_eval_episode))                                 
                
        if (epoch_step) % args.save_model_freq == 0:
            torch.save({
                'epoch': epoch_step, 
            ...
            }, os.path.join(save_model_dir, 'EnsembleDynamicsModel_{}.pt'.format(epoch_step)))
            torch.save({
                'epoch': epoch_step, 
            ...
            }, os.path.join(save_policy_dir, 'PolicyNetwork_{}.pt'.format(epoch_step)))
            ...
            # torch.save(predict_)
            env_pool.save(os.path.join(save_env_pool_dir, '{}.pkl'.format(epoch_step)))
            model_pool.save(os.path.join(save_dynamics_pool_dir, '{}.pkl'.format(epoch_step)))
            scalar_mu_std = [predict_env.model.scaler.mu, predict_env.model.scaler.std]
            with open(os.path.join(args.save_dir, 'scaler_mu_std_{}.pkl'.format(epoch_step)), 'wb') as f:
                pickle.dump(scalar_mu_std, f)
        print(f'epoch: {epoch_step} finished')
        # logging.info('Epoch step: ' + str(epoch_step) + ' training finished!')
        # save the replay buffer and scaler statistics fo standardization
    env_pool.save(os.path.join(save_env_pool_dir, '{}.pkl'.format(epoch_step)))
    model_pool.save(os.path.join(save_dynamics_pool_dir, '{}.pkl'.format(epoch_step)))
    scalar_mu_std = [predict_env.model.scaler.mu, predict_env.model.scaler.std]
    with open(os.path.join(args.save_dir, 'scaler_mu_std_{}.pkl'.format(epoch_step)), 'wb') as f:
...
def test_policy_dependent_models(args, env, state_size, action_size, start_eval, end_eval, save_model_dir, save_policy_dir):
    ...
    state_error_dict = {}
    reward_error_dict = {}
    with open(os.path.join(args.save_dir, 'scaler_mu_std_{}.pkl'.format(str(int(args.num_epoch-1)))), 'rb') as f:
        mean, std = pickle.load(f)
    for model_epoch in checkpoint_epochs:
        dynamics_model_checkpoint = torch.load(str(os.path.join(save_model_dir, 'EnsembleDynamicsModel_{}.pt'.format(model_epoch))))
        ...
        for policy_epoch in checkpoint_epochs:
            policy_network_checkpoint = torch.load(str(os.path.join(save_policy_dir, 'PolicyNetwork_{}.pt'.format(model_epoch))))
    ...
    with open(str(os.path.join(args.save_dir, args.env_name, 'model_policy_return_dict_{}_{}_{}'.format(start_eval, save_freq, end_eval))), 'w') as f:
        json.dump(model_policy_return_dict, f)
    with open(str(os.path.join(args.save_dir, args.env_name, 'state_error_dict_{}_{}_{}.json'.format(start_eval, save_freq, end_eval))), 'w') as f:
        json.dump(state_error_dict, f)
    with open(str(os.path.join(args.save_dir, args.env_name, 'reward_error_dict_{}_{}_{}.json'.format(start_eval, save_freq, end_eval))), 'w') as f:
...
def main(args=None):
    ...
    save_model_dir = os.path.join(args.save_dir, args.env_name, 'dynamics_model')
    save_policy_dir = os.path.join(args.save_dir, args.env_name, 'policy_network')
    save_env_buffer_dir = os.path.join(args.save_dir, args.env_name, 'env_buffer')
    save_dynamics_buffer_dir = os.path.join(args.save_dir, args.env_name, 'dynamics_buffer')
    ...
    if 'Ant' in args.env_name:
        ...
        print('Loaded TruncatedObs-version of the Ant environment: {}'.format(args.env_name))
    # else:
    #     env_name = args.env_name
    env = gym.make(args.env_name)
    
    job_name = 'MBPO_test_policy_dependent_models_{}_{}_{}'.format(args.env_name, args.model_type, args.seed)
    writer = SummaryWriter(str(os.path.join(args.save_dir, 'tensorboard', job_name)))
    ...
    train(args, env_sampler, predict_env, agent, env_pool, model_pool, writer, save_model_dir, save_policy_dir, save_env_buffer_dir, save_dynamics_buffer_dir)
    
    print('Training complete!')
    print('---------------------------------------------------------------------')
    print('Start evaluating different policies at different model checkpoints...')
    print('---------------------------------------------------------------------')
    test_policy_dependent_models(args, env, state_size, action_size, args.save_model_freq, args.save_model_freq*6, save_model_dir, save_policy_dir)  
