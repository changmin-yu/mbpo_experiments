...
class EnsembleModel(nn.Module):
    def __init__(self, state_size, action_size, reward_size, ensemble_size, hidden_size=200, learning_rate=1e-3, use_decay=False, predict_done=False):
        ...
        # Add variance output
        if predict_done:
            self.nn5 = EnsembleFC(hidden_size, self.output_dim * 2+1, ensemble_size, weight_decay=0.0001)    
        else:
    ...
    def forward(self, x, ret_log_var=False):
        ...
        if self.predict_done:
            logvar = self.max_logvar - F.softplus(self.max_logvar - nn5_output[:, :, self.output_dim:-1])
        else:
    ...
    def loss(self, mean, logvar, labels, inc_var_loss=True, done=None):
        """
        ...
        if self.predict_done:
            ...
            done_labels = labels[..., -1]
            labels = labels[..., :-1]
            done_loss = F.binary_cross_entropy(done, done_labels)
        if inc_var_loss:
        ...
        if done is not None:
            total_loss = total_loss + torch.sum(done_loss)
            mse_loss = mse_loss + done_loss
...
class EnsembleDynamicsModel():
    def __init__(self, network_size, elite_size, state_size, action_size, reward_size=1, hidden_size=200, use_decay=False, predict_done=False):
        ...
        self.ensemble_model = EnsembleModel(state_size, action_size, reward_size, network_size, hidden_size, use_decay=use_decay, predict_done=predict_done)
    ...
    def train(self, inputs, labels, batch_size=256, holdout_ratio=0., max_epochs_since_update=5):
        ...
        for epoch in itertools.count():
            ...
            for start_pos in range(0, train_inputs.shape[0], batch_size):
                ...
                mean, logvar, done = self.ensemble_model(train_input, ret_log_var=True)
                loss, _ = self.ensemble_model.loss(mean, logvar, train_label, done=done)
                self.ensemble_model.train(loss)
                losses.append(loss)
            train_losses_list.append(torch.mean(torch.tensor(losses)).item())

            with torch.no_grad():
                holdout_mean, holdout_logvar, holdout_done = self.ensemble_model(holdout_inputs, ret_log_var=True)
                _, holdout_mse_losses = self.ensemble_model.loss(holdout_mean, holdout_logvar, holdout_labels, inc_var_loss=False, done=holdout_done)
                holdout_mse_losses = holdout_mse_losses.detach().cpu().numpy()
                holdout_mse_losses_list.append(holdout_mse_losses)
                sorted_loss_idx = np.argsort(holdout_mse_losses)
                self.elite_model_idxes = sorted_loss_idx[:self.elite_size].tolist()
                break_train = self._save_best(epoch, holdout_mse_losses)
                if break_train:
                    break
        # return the mean training loss and mean holdout mse losses over the training episodes
        return torch.mean(torch.tensor(train_losses_list)).item(), torch.mean(torch.tensor(holdout_mse_losses_list), dim=0)
    ...
    def predict(self, inputs, batch_size=1024, factored=True):
        ...
        for i in range(0, inputs.shape[0], batch_size):
            ...
            b_mean, b_var, b_done = self.ensemble_model(input[None, :, :].repeat([self.network_size, 1, 1]), ret_log_var=False)
        ...
        if self.predict_done:
            ensemble_done = np.hstack(ensemble_done)
        else:
            ensemble_done = None
            
        if factored:
            return ensemble_mean, ensemble_var, ensemble_done
        else:
