...
def train(args, env_sampler, predict_env, agent, env_pool, model_pool):
    ...
    for epoch_step in range(20, 40):
        ...
        print('epoch {} done'.format(epoch_step))
        if (epoch_step) % 2 == 0 and epoch_step > 20:
            torch.save({
                'epoch': epoch_step, 
            ...
            }, args.save_model_path+'EnsembleDynamicsModel_'+str(epoch_step)+'.pt')
            torch.save({
                'epoch': epoch_step, 
            ...
            }, args.save_policy_path+'PolicyNetwork_'+str(epoch_step)+'.pt')
            ...
            # torch.save(predict_)
            env_pool.save(args.env_pool_save_fname+str(epoch_step)+'.pkl')
            model_pool.save(args.model_pool_save_fname+str(epoch_step)+'.pkl')
            scalar_mu_std = [predict_env.model.scaler.mu, predict_env.model.scaler.std]
            with open('./test_policy_dependent_results_2/scaler_mu_std_{}.pkl'.format(epoch_step), 'wb') as f:
                pickle.dump(scalar_mu_std, f)
            print('checkpoint_{} saved'.format(epoch_step))
        # logging.info('Epoch step: ' + str(epoch_step) + ' training finished!')
        # save the replay buffer and scaler statistics fo standardization
    env_pool.save(args.env_pool_save_fname+str(epoch_step)+'.pkl')
    model_pool.save(args.model_pool_save_fname+str(epoch_step)+'.pkl')
    scalar_mu_std = [predict_env.model.scaler.mu, predict_env.model.scaler.std]
    with open('./test_policy_dependent_results_2/scaler_mu_std_40.pkl', 'wb') as f:
        pickle.dump(scalar_mu_std, f)
...
def test_policy_dependent_models(args, env, state_size, action_size, env_sampler, writer):
        ...
        for policy_epoch in checkpoint_epochs:
            ...
            for i in range(args.num_eval_episode):
                ...
                while not done:
                    ...
                    sum_reward += reward
                    counter += 1
                logging.info('Policy epoch{} | DynamicsModel epoch{} | number of steps: {} | inner eval num: {} | sum reward: {}'.format(counter, i, sum_reward))
            ...
            model_policy_return_dict['model_{}_policy_{}'.format(model_epoch, policy_epoch)] = [mean_episode_reward, std_episode_reward]
    with open('test_policy_dependent_results_2/mean_std_evaluated_policy.json', 'w') as f:
...
def main(args=None):
    ...
    # Intial agent
    agent = SAC(env.observation_space.shape[0], env.action_space, args)
    policy_network_checkpoint = torch.load('./test_policy_dependent_results_2/policy/PolicyNetwork_20.pt')
    agent.policy.load_state_dict(policy_network_checkpoint['policy_model_state_dict'])
    ...
    dynamics_model_checkpoint = torch.load('./test_policy_dependent_results_2/dynamics_model/EnsembleDynamicsModel_20.pt')
    env_model.ensemble_model.load_state_dict(dynamics_model_checkpoint['dynamics_model_state_dict'])
    ...
    env_pool.load('./test_policy_dependent_results_2/env_buffer/env_buffer_20.pkl')
    ...
    model_pool.load('./test_policy_dependent_results_2/model_buffer/model_buffer_20.pkl')
    ...
    # print('---------------------------------------------------------------------')
    # test_policy_dependent_models(args, env, state_size, action_size, env_sampler, writer)    
